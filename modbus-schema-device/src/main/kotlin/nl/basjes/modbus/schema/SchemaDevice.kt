...
open class SchemaDevice @JvmOverloads constructor(
    /**
    ...
    fun getRegisterBlock(addressClass: AddressClass): RegisterBlock {
        return registerBlocks.computeIfAbsent(addressClass) {
            addrCls -> RegisterBlock(addrCls)
        }
    }
    ...
    fun getBlock(blockId: String): Block? {
        return mutableBlocksMap[blockId]
    }
    ...
    @JvmOverloads
    fun connect(modbusDevice: ModbusDevice,
                /**
    ...
    fun createTestsUsingCurrentRealData() {
        ...
        val description = "Test generated from device data at " +
    ...
    fun verifyProvidedTests(): TestScenarioResultsList {
        ...
        for (test in tests) {
            ...
            for (expectedBlock in test.expectedBlocks) {
                ...
                requireNotNull(block) { "There are expectations for the blockid \"${expectedBlock.blockId}\" in the test \"${test.name}\" which does not exist." }
    ...
    private fun toTable(table: StringTable, onlyUseFullFields: Boolean) {
        var first = true
        for (block in blocks) {
            if (!first) {
    ...
    open class SchemaDeviceBuilder {
         ...
         * The maximum number of registers per request is different for some devices
         */
        fun maxRegistersPerModbusRequest(maxRegistersPerModbusRequest: Int) = apply { this.maxRegistersPerModbusRequest = maxRegistersPerModbusRequest }
